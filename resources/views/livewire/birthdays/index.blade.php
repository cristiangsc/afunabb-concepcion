<div>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-x-10">
            <x-principal.header title="AFUNABB" subtitle="Saludo de Cumpleaños"/>
            <x-fecha-hoy/>
        </div>
    </x-slot>
                        ...
                        <div class="border-b border-gray-900/10 pb-12">
                            <div class="bg-green-800  flex items-center justify-center  py-2">
                                ...
                                @can('cumpleaños create')
                                    <button
                                        ...
                                        <svg class="h-7 w-7 text-green-600" xmlns="http://www.w3.org/2000/svg"
                                             fill="none"
                            ...
                            <div
                                ...
                                @foreach($saludos as $saludo)
                                    <div wire:key="{{ $saludo->id }}"
                                         class="col-span-1 flex flex-col overflow-hidden shadow-lg">
                                        @if($saludo->getFirstMediaUrl('saludos'))
                                            <img class="w-full h-48 object-cover"
                                                 src="{{$saludo->getFirstMediaUrl('saludos')}}"
                                                 alt="{{$saludo->mensaje}}">
                                        @endif
                                        <p class="ml-2 text-xs font-semibold p-4 uppercase">Mensaje: {{$saludo->mensaje}}</p>
                                        <div class="flex flex-wrap mt-auto">
                                            @can('cumpleaños delete')
                                                <x-button type="button" wire:click="deleteSaludo({{$saludo->id}})"
