<div>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-x-10">
            <x-principal.header title="AFUNABB" subtitle="Otros egresos"/>
            <x-fecha-hoy/>
        </div>
    </x-slot>
    ...
    <div class="grid-container">
        ...
        <x-table-dinamic caption="OTROS EGRESOS">
            ...
            @foreach($egresos as $egreso)
                <tr wire:key="{{ $egreso->id }}" class="border-b border-gray-300 hover:bg-gray-100">
                    <td class="py-4 px-2 text-left whitespace-nowrap border-dashed border-t border-gray-200">
                        <span class="text-gray-700"> {{ $egreso->egreso->name }} </span>
                    </td>
                    <td class="py-4 px-2 text-right whitespace-nowrap border-dashed border-t border-gray-200">
                        <span class="text-gray-700"> {{numMiles($egreso->monto)}} </span>
                    </td>
                    <td class="text-center whitespace-nowrap border-dashed border-t border-gray-200">
                        <span class="text-gray-700">{{$egreso->fecha}}</span>
                    </td>
                    <td class="text-left whitespace-nowrap border-dashed border-t border-gray-200">
                        <span class="text-gray-700"> {{  Str::limit(strip_tags($egreso->observacion),25,'') }}</span>
                        <span wire:click="OpenModalObservacion('{{ $egreso->observacion }}')"
                    ...
                    <td class="text-center whitespace-nowrap border-dashed border-t border-gray-200">
                        ...
                        @endcan
                        @can('egresos delete')
                            <span wire:click="deleteEgresoVarios({{$egreso->id}})"
