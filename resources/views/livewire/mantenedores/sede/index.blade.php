<div>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-x-10">
            <x-principal.header title="AFUNABB" subtitle="Mantenedor de sedes"/>
            <x-fecha-hoy/>
        </div>
    </x-slot>
            ...
            <x-table-dinamic caption="SEDES">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-6 py-2 cursor-pointer sticky top-0">
                        @can('sedes create')
                            <button
                                wire:click="OpenModalSedeCreate()"
                                type="button"
                ...
                @foreach($sedes as $sede)
                    <tr wire:key="{{ $sede->id }}" class="border-b border-gray-300 hover:bg-gray-100">

                        <td class="py-4 px-2 text-left whitespace-nowrap border-dashed border-t border-gray-200">
                            <span class="text-gray-700">{{$sede->name}}</span>
                        </td>
    ...
    <x-modal-dinamic
        openCloseModal={{$OpenCloseModal}} title={{$title}}  actionTarget={{$actionTarget}} button={{$buttonSede}}>
        <x-component-input
            label=""
            placeholder=""
            name="name"
