<div>
        ...
        <x-table-dinamic caption="BENEFICIOS SOCIALES">
            ...
            @foreach($benefits as $benefit)
                <tr class="border-b border-gray-300 hover:bg-gray-100">
                    ...
                    <td class="text-center whitespace-nowrap border-dashed border-t border-gray-200">
                        <span
                            class="{{$benefit->vigencia ?'bg-purple-500':'bg-yellow-500'}} text-white py-1 px-3 rounded-full text-xs">{{$benefit->vigencia ? 'Vigente' : ' No vigente'}}</span>
                    </td>
                    <td class="text-center whitespace-nowrap border-dashed border-t border-gray-200">
                        @can('beneficios update')
                            <x-button type="button" wire:click="OpenModalBeneficioCreate({{ $benefit->id }})"
    ...
    <x-dialog-modal wire:model="showModal">
        ...
        <x-slot name="title">
            <div class="bg-green-800 text-white text-sm text-center border-b rounded-t-lg"> Beneficios</div>
        </x-slot>
