<div>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-x-10">
            <x-principal.header title="AFUNABB" subtitle="Fotografía directiva"/>
            <x-fecha-hoy/>
        </div>
    </x-slot>
                        ...
                        <div class="border-b border-gray-900/10 pb-12">
                            ...
                            <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2  md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 justify-items-center">
                                @foreach($photos as $photo)
                                    <div wire:key="{{ $photo->id }}"
                                         class="col-span-1 flex flex-col overflow-hidden shadow-lg">
                                        @if($photo->getFirstMediaUrl('directivas'))
                                            <img class="w-full h-48 object-cover"
                                                 src="{{$photo->getFirstMediaUrl('directivas')}}"
                                                 alt="{{$photo->title}}">
                                        @endif
                                        <p class="ml-2 text-xs font-semibold">TÍTULO: {{$photo->title}}</p>
                                        <div class="flex flex-wrap mt-auto">
                                            <x-button type="button" wire:click="deletePhoto({{$photo->id}})"
                                                      wire:confirm="¿Desea eliminar esta fotografía?"
                                                      wire:stop
    ...
    <x-dialog-modal wire:model="showModal" :maxWidth="'6xl'">
        ...
        <x-slot name="content">
            <div class="space-y-6">
                <div class="border-b border-gray-900/10 pb-12">
                    <x-component-input
                        placeholder="Ingrese el Título de la fotografía"
